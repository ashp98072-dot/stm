alter table public.quote_items add column if not exists variant_id uuid;
do $$ begin alter table public.quote_items add constraint quote_items_variant_same_org foreign key(organization_id,variant_id)references public.product_variants(organization_id,id)on delete set null;exception when duplicate_object then null;end $$;
create index if not exists quote_items_variant_idx on public.quote_items(variant_id)where variant_id is not null;
...
begin
 ...
 update quote_lines target set product_name=case when variant.id is null then product.name else product.name||' · '||variant.name end,sku=coalesce(variant.sku,product.sku),unit_price=coalesce(variant.price,public.product_rule_price(p_organization_id,product.id,target.quantity,now())),tax_rate=product.tax_rate from public.products product left join public.product_variants variant on variant.id=target.variant_id and variant.product_id=product.id and variant.organization_id=p_organization_id and variant.active where product.id=target.product_id and product.organization_id=p_organization_id and product.active and(target.variant_id is null or variant.id is not null);
