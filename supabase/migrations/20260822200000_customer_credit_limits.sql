...
create or replace function public.enforce_customer_credit_limit()
...
begin
  ...
  select credit_limit into allowed_limit from public.customers where id=new.customer_id;
  if allowed_limit is null then return new; end if;
  select coalesce(sum(case when type='charge' then amount else -amount end),0)
    into current_balance from public.customer_account_movements where customer_id=new.customer_id;
  if current_balance+new.amount>allowed_limit then raise exception 'customer credit limit exceeded'; end if;
  return new;
end;
$$;

create trigger customer_credit_limit_guard
