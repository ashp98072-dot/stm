...
begin
  ...
  if p_quantity < 0 or p_reorder_point < 0 or length(trim(p_reason)) < 3 then
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_location_id::text || p_product_id::text, 2)
  );
  ...
  insert into public.inventory_levels (
    ...
    p_organization_id, p_location_id, p_product_id, p_quantity, p_reorder_point
  )
  ...
  if quantity_delta <> 0 then
    insert into public.inventory_movements (
      ...
      p_organization_id, p_location_id, p_product_id, quantity_delta, trim(p_reason), auth.uid()
    );
  end if;
