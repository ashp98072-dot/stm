...
import { ArrowLeft, Boxes, GitBranch, Search } from "lucide-react";
          ...
          {!products?.length ? (
            ...
            return (
              <article key={product.id} className="grid gap-4 border-b border-black/8 px-5 py-4 last:border-0 md:grid-cols-[minmax(220px,2fr)_1fr_1fr_1fr_1.4fr] md:items-center">
                <div><p className="font-bold">{product.name}</p><p className="mt-1 text-xs text-[#77847d]">{product.sku || "Sin SKU"}{product.barcode ? ` · ${product.barcode}` : ""}</p><Link href={`/inventario/${product.id}/variantes`} className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[#285645] hover:underline"><GitBranch size={13}/>Variantes</Link></div>
