...
export default async function InventoryPage({ searchParams }: PageProps<"/inventario">) {
  ...
  let productsQuery = context.supabase
    .from("products")
    .select("id, name, sku, barcode, cost, price, tax_rate, active, manufacturer_id, category:categories(name), manufacturer:manufacturers!products_manufacturer_same_org(name), product_tags!product_tags_organization_id_product_id_fkey(tag_id), inventory_levels(quantity, reorder_point, location_id)")
