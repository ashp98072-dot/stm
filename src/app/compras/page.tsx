...
export default async function PurchasesPage({ searchParams }: PageProps<"/compras">) {
  ...
  const [{ data: products }, { data: rawSuppliers }, { data: purchases }, { data: supplierMovements }] = await Promise.all([
    ...
    context.supabase.from("suppliers").select("id, name, credit_limit").eq("organization_id", context.organization.id).eq("active", true).order("name"),
    context.supabase.from("purchases").select("id, reference, status, payment_terms, total, received_at, supplier:suppliers(name)").eq("organization_id", context.organization.id).eq("location_id", context.location.id).order("received_at", { ascending: false }).limit(10),
    context.supabase.from("supplier_account_movements").select("supplier_id,type,amount").eq("organization_id", context.organization.id),
  ]);
