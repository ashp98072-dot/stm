...
export async function returnPurchaseItems(formData: FormData) {
  ...
  if (error) { const message = error.message.toLowerCase(); redirect(`/devoluciones-compras?purchase=${parsed.data.purchaseId}&error=${message.includes("open cash register") ? "register" : message.includes("stock") ? "stock" : message.includes("paid") ? "paid" : message.includes("exceeds") ? "quantity" : "save"}`); }
