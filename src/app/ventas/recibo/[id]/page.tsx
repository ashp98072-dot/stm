...
export default async function ReceiptPage({ params, searchParams }: PageProps<"/ventas/recibo/[id]">) {
  ...
  const { data: sale } = await context.supabase.from("sales")
    .select("id, receipt_number, status, cashier_id, subtotal, tax_total, total, completed_at, location:locations(name), customer:customers(first_name, last_name, company_name, tax_id), sale_items(product_name, sku, quantity, unit_price, tax_total, line_total), payments(method, amount, reference), sale_voids(reason, voided_at)")
