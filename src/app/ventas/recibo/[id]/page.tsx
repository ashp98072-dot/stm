...
export default async function ReceiptPage({ params }: PageProps<"/ventas/recibo/[id]">) {
  ...
  const { data: sale } = await context.supabase.from("sales")
    .select("id, receipt_number, subtotal, tax_total, total, completed_at, location:locations(name), customer:customers(first_name, last_name, company_name, tax_id), sale_items(product_name, sku, quantity, unit_price, tax_total, line_total), payments(method, amount, reference)")
