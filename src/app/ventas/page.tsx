...
export default async function SalesPage({ searchParams }: PageProps<"/ventas">) {
  ...
  const { data: quote } = quoteId ? await context.supabase.from("quotes").select("id, customer_id, status, quote_items(product_id, variant_id, quantity)").eq("id", quoteId).eq("organization_id", context.organization.id).eq("status", "draft").maybeSingle() : { data: null };
  const quoteItems = (quote?.quote_items ?? []) as Array<{ product_id:string|null; variant_id:string|null; quantity:string|number }>;
